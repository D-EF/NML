 ...
 * @LastEditTime: 2023-02-24 00:00:17
...
import { CONFIG, DEG_180, DEG_90, SAFE_MATH_TOOLS, NML_VALUE_TYPE, approximately } from "../Config__NML.js";
import { Matrix } from "./Matrix.js";
import { Matrix_3D } from "./Matrix_3D.js";
import {Vector} from "./Vector.js";

/*h*/const {sin,cos,asin,acos,abs,sqrt,tan,atan2}=SAFE_MATH_TOOLS;

// open * 部分映射函数和映射表 * open

    /** @type {Int8Array} 对应轴向[z,x,y] (BPH)的旋转矩阵的空行/列的uv*/
    const _MAPPING__EulerAngles_NULL_UV_MATRIX=new Int8Array([2,0,1]);

    /** @type {Int8Array} 左手坐标系的 对应轴向[z,x,y] (BPH)的旋转矩阵的-sin的下标*/
    const _MAPPING__EulerAngles_INVERSE_INDEX_MATRIX__LEFT=new Int8Array([3,7,2]);

    /** @type {Int8Array} 右手坐标系的 对应轴向[z,x,y] (BPH)的旋转矩阵的-sin的下标*/
    const _MAPPING__EulerAngles_INVERSE_INDEX_MATRIX__RIGHT=new Int8Array([1,5,6]);

    /** @type {Int8Array} 典型欧拉角的cos值uv值映射 */
...
/** 欧拉角 */
class Euler_Angles extends NML_VALUE_TYPE{
    ...
    /**
     * @template {Euler_Angles|List_Value} T
     * @param {Matrix_3D} mat 4x4 旋转矩阵
     * @param  {int[]}      [_axis] 创建旋转矩阵时的乘法顺序 [z,x,y] 默认为 [0,1,2] (BPH)(zxy)
     * @param  {T} [_out]  接收数据的对象
     * @return {T|Euler_Angles} 修改并返回 out, 或返回一个新的欧拉角
     */
    ...
    /** 使用矩阵生成欧拉角
     * @template T
     * @param  {Matrix|List_Value}      mat    仅做过旋转变换的3x3矩阵
     * @param  {int[]}      [_axis] 创建旋转矩阵时的乘法顺序 [z,x,y] 默认为 [0,1,2] (BPH)(zxy)
     * @param  {T} [_out]  接收数据的对象
     * @return {T|Euler_Angles} 修改并返回 out, 或返回一个新的欧拉角
     */
